...
- [x] Wallet nonce and verification bodies execute OpenAPI 0.8.9's shared
...
- [x] OpenAPI 0.8.9 and the runtime domain-bind evaluator/tester signatures
...
- [x] OpenAPI 0.8.9 binds all 185 advertised 4xx/5xx responses across 37
...
- [x] Agent authentication parses one bounded ASCII Agent ID (3–120 characters)
  ...
  the same 401 failure. OpenAPI 0.8.9 publishes exact constraints and packaged
...
- [x] All 18 production dynamic-path operations execute shared UUID, positive
  ...
  chain work, with exact OpenAPI 0.8.9 bounds and a closed 400 response.
...
- [x] Hidden-test ciphertext upload uses one authenticated, manifest-bound shared
  ...
  media/encoding is 415. OpenAPI 0.8.9, unit tests and packaged production smoke
...
- [x] All eleven Agent job kinds use one closed runtime union across PostgreSQL
  ...
  OpenAPI 0.8.9. Each kind has an exact role and bounded payload; partial chain
...
- [x] Every Agent job kind has one closed OpenAPI 0.8.9 and runtime completion-
...
- [x] AI dashboard schema 2.0 enumerates all 37 production OpenAPI 0.8.9
...
- [x] OpenAPI 0.8.9 and the well-known manifest cover every advertised public,
...
- [x] Well-known discovery schema 1.1 gives every production SDK HTTP workflow
  ...
  discovered API template exists in OpenAPI 0.8.9; packaged production smoke
...
- [ ] Agent quality is role-separated and derived only from canonical outcomes.
  ...
  to hold replay-protected reporter authority for all three roles; Dashboard 2.0,
  OpenAPI 0.8.9 and the shared ABI expose the policy and entrypoint. The row
...
- [ ] Indexer, OpenAPI, SDK and the human/AI dashboards expose source attribution,
  ...
  Dashboard schema 2.0/OpenAPI 0.8.9 now expose the exact 50/40/10 and
