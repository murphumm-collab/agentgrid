...
- [x] Wallet nonce and verification bodies execute OpenAPI 0.8.14's shared
...
- [x] OpenAPI 0.8.14 and the runtime domain-bind evaluator/tester signatures
...
- [x] OpenAPI 0.8.14 binds all 185 advertised 4xx/5xx responses across 37
...
- [x] Agent authentication parses one bounded ASCII Agent ID (3–120 characters)
  ...
  the same 401 failure. OpenAPI 0.8.14 publishes exact constraints and packaged
...
- [x] All 18 production dynamic-path operations execute shared UUID, positive
  ...
  chain work, with exact OpenAPI 0.8.14 bounds and a closed 400 response.
...
- [x] Hidden-test ciphertext upload uses one authenticated, manifest-bound shared
  ...
  media/encoding is 415. OpenAPI 0.8.14, unit tests and packaged production smoke
...
- [x] All sixteen Agent job kinds use one closed runtime union across PostgreSQL
  ...
  OpenAPI 0.8.14. Each kind has an exact role and bounded payload; partial chain
...
- [x] Every Agent job kind has one closed OpenAPI 0.8.14 and runtime completion-
...
- [x] Evaluator and validator selection remains executable at the governed
  ...
  post-entropy manipulation. A pool ending below three live conflict-free
  winners records its objective exhaustion version and can create one
  deterministic successor only after a later registry version; the successor
  derives entropy from the already observed predecessor and cannot obtain a new
  future block. Task-path regression covers evaluator and validator recovery.
  A test-only sparse-prefix harness executes the exact production last-page and
  Fenwick-prune code paths at the governed 65,536-Agent boundary; both the final
  64-candidate build page and a 16-prune draw remain below the shared
  30,000,000-gas transaction budget. The harness changes no production method
  and does not replace the all-candidate prefix. A small random window that
  excludes most eligible candidates does not satisfy this row.
...
- [x] AI dashboard schema 2.4 enumerates all 37 production OpenAPI 0.8.14
...
- [x] OpenAPI 0.8.14 and the well-known manifest cover every advertised public,
...
- [x] Well-known discovery schema 1.1 gives every production SDK HTTP workflow
  ...
  discovered API template exists in OpenAPI 0.8.14; packaged production smoke
...
- [ ] Agent quality is role-separated and derived only from canonical outcomes.
  ...
  OpenAPI 0.8.14 and the shared ABI expose the policy and entrypoint. The row
...
- [ ] Indexer, OpenAPI, SDK and the human/AI dashboards expose source attribution,
  ...
  Dashboard schema 2.5/OpenAPI 0.8.14 now expose the exact 50/40/10 and
