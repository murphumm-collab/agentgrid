...
- 随机测试者被分配后有 24 小时链上期限；到期未提交时，任何地址都可触发 1% 罚没、释放任务锁、
  将该钱包排除出本任务并基于新的未来区块重新随机选择测试者，任务成果和执行进度不得丢失。

### 3.8 接受、拒绝、申诉和维护

- 发布者接受后才创建奖励 Grant、开放交付成果并进入 Maintenance。
- 独立测试通过后发布者有 72 小时确认或结构化拒绝；到期静默时，任何地址都可按已冻结完成标准
  自动接受并创建原有上限内的 Grant，`UserReviewed` 的 reason code `0x01` 标记该自动路径。
