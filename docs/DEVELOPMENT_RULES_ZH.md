...
- 选择池只有在 cursor 到达完整冻结候选数量后才能安排未来随机区块；分页必须连续且有上限，候选地址与冻结权重审计行不得因抽签/剔除而改写，当前失效或冲突候选的移除必须以有界、可续跑状态持久化。仅实现选择池原语不算完成，评估者和验证者 TaskRegistry 路径、部署接线、索引器/Worker 重试与最坏 BSC gas 证据必须全部使用它。
