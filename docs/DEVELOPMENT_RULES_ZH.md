...
- 选择池只有在 cursor 到达完整冻结候选数量后才能安排未来随机区块；分页必须连续且有上限，候选地址与冻结权重审计行不得因抽签/剔除而改写，当前失效或冲突候选的移除必须以有界、可续跑状态持久化。评估者和验证者 TaskRegistry 路径、部署接线与索引器/Worker 必须使用同一选择池；只有候选池耗尽后能依据客观 registry 变更恢复、不能通过重抽打磨随机数，且最大规模 BSC gas 回归通过，才能关闭可扩展性门禁。
