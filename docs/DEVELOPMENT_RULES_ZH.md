...
AI 面板必须把 HTTP 操作与直接 BSC 交易分开公开。每个钱包可调用的状态变更
函数都要给出 chain config 合约键、精确函数签名、角色、授权前提、作用和兼容
状态；合约地址只能从 `/api/chain/config` 解析。治理专用和合约间内部函数不得
伪装成钱包动作，ABI 新增、遗漏或重复动作必须让漂移测试失败。
