...
the OpenAPI 0.8.12 machine surface and economics dashboard projection pass locally. The
...
validator result are now removed as well; OpenAPI 0.8.12 uses `testerIds`,
...
and never suppress failures. Dashboard 2.3/OpenAPI 0.8.12 publish this boundary.
...
Dashboard 2.3/OpenAPI 0.8.12 expose the closed anti-manipulation and public
...
an RPC or write failure can no longer silently replace validators. OpenAPI 0.8.12
...
26/26 cases across four files, including decoded event assertions. The
...
Dashboard schema 2.3/OpenAPI 0.8.12 now separate the complete 37-operation HTTP
contract from the compiled chain surface. All 102 mutable signatures across the
nine deployment artifacts are partitioned into 46 supported participant actions
and 56 explicit governance-only, protocol-internal or generic-token exclusions.
...
to submit or prune candidates. Dashboard schema 2.3/OpenAPI 0.8.12 publish the
...
One local scalability gate remains explicitly open: `AgentRegistry` currently
scans the frozen candidate prefix twice for each selected slot and performs
cross-contract eligibility/conflict reads, so worst-case gas grows linearly with
the append-only registry. The next valid design must bound each transaction while
retaining every frozen eligible candidate's committed weight and preventing a
builder from stopping early or learning entropy before choosing pages. A small
random scan window would improve gas by silently weakening open access and is not
...
2.3/OpenAPI 0.8.12 expose `AgentRegistry.frozenSelectionWeightAt` as the
...
`AgentRegistry` now also contains the next bounded primitive: a protocol-created
selection pool freezes the complete request-time candidate prefix, anyone may
...
weights remain unchanged. This is not yet the production task path: TaskRegistry,
deployment wiring, indexer jobs, coordinator retries and a governed maximum-size
BSC gas regression remain open. Dashboard 2.3 reports that distinction directly.
...
Dashboard schema 2.3 and OpenAPI 0.8.12 now expose the exact advertising and
...
Dashboard schema 2.3/OpenAPI 0.8.12 additionally expose receipt-bound signed
