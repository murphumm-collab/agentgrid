...
the OpenAPI 0.8.13 machine surface and economics dashboard projection pass locally. The
...
validator result are now removed as well; OpenAPI 0.8.13 uses `testerIds`,
...
and never suppress failures. Dashboard 2.4/OpenAPI 0.8.13 publish this boundary.
...
contract run now passes all 29 cases across five files in one uninterrupted run,
...
Dashboard 2.4/OpenAPI 0.8.13 expose the closed anti-manipulation and public
...
an RPC or write failure can no longer silently replace validators. OpenAPI 0.8.13
...
29/29 cases across five files, including decoded event assertions and the
...
Dashboard schema 2.4/OpenAPI 0.8.13 now separate the complete 37-operation HTTP
...
to submit or prune candidates. Dashboard schema 2.4/OpenAPI 0.8.13 publish the
...
One local scalability gate remains explicitly open. `AgentRegistry` now contains
...
bounded Coordinator retries. The gate remains open because a pool exhausted
below three live conflict-free winners has no objective registry-change recovery
path yet, and the governed maximum-size BSC gas regression has not run. No fixed
release QA is warranted while these source and state-machine gates remain open.
...
Dashboard schema 2.4 and OpenAPI 0.8.13 now expose the exact advertising and
...
Dashboard schema 2.4/OpenAPI 0.8.13 additionally expose receipt-bound signed
