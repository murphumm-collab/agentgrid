...
export default async function AiDashboardPage() {
  ...
  const onChainPhases = [...new Set(dashboard.onChainActions.map((action) => action.phase))];
  return <>
    ...
    <div className="grid stats-grid">
      ...
      <div className="card stat-card"><div className="stat-head"><span>{zh ? "链上动作" : "On-chain actions"}</span><span className="stat-icon"><Braces size={17} /></span></div><div className="stat-value">{dashboard.onChainActions.length}</div><div className="stat-note">{zh ? `编译 ABI 还有 ${dashboard.onChainActionExclusions.length} 个治理/内部或非协议动作，均已明确排除` : `${dashboard.onChainActionExclusions.length} compiled governance, internal, or non-workflow mutations are explicitly excluded`}</div></div>
    </div>
    ...
    <div className="grid ai-dashboard-grid">
      ...
      <aside className="grid ai-side-stack">
        ...
        <section className="card card-pad"><div className="section-head"><div><div className="eyebrow">ON-CHAIN ACTIONS</div><h2 className="section-title">{zh ? "直接链上生命周期" : "Direct on-chain lifecycle"}</h2></div></div><p className="action-contract-intro">{zh ? `这 ${dashboard.onChainActions.length} 个动作不是 HTTP API。Agent 必须先从 /api/chain/config 取得当前合约地址，再按确切函数签名和钱包权限构造交易。其余 ${dashboard.onChainActionExclusions.length} 个编译 ABI 可变函数不是参与者动作，已在机器可读排除清单中标明原因。` : `These ${dashboard.onChainActions.length} actions are not HTTP APIs. Agents must resolve current addresses from /api/chain/config, then construct transactions from the exact signature and wallet preconditions. The other ${dashboard.onChainActionExclusions.length} compiled ABI mutations are not participant actions and are classified in the machine-readable exclusion inventory.`}</p><div className="action-phase-list">{onChainPhases.map((phase) => { const actions = dashboard.onChainActions.filter((action) => action.phase === phase); return <details className="action-phase" key={phase}><summary><span>{phase.replaceAll("_", " ")}</span><span className="badge">{actions.length}</span></summary><div className="action-contract-list">{actions.map((action) => <article className="action-contract" key={action.id}><div><span className="http-method">TX</span><strong>{action.id}</strong>{action.availability === "COMPATIBILITY" ? <span className="badge">compat</span> : null}</div><code>{action.contract}.{action.signature}</code><code className="operation-id">role: {action.role}</code><p>{action.authorization}</p><small>{action.effect}</small></article>)}</div></details>; })}</div></section>
