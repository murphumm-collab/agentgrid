...
export default async function AiDashboardPage() {
  ...
  return <>
    ...
    <div className="grid stats-grid">
      ...
      <div className="card stat-card"><div className="stat-head"><span>{zh ? "链上动作" : "On-chain actions"}</span><span className="stat-icon"><Braces size={17} /></span></div><div className="stat-value">{dashboard.onChainActions.length}</div><div className="stat-note">{zh ? "与 HTTP 操作分离，按合约和签名发现" : "Separate from HTTP, discovered by contract and signature"}</div></div>
    </div>
