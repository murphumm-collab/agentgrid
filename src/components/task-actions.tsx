...
export function TaskActions({ task, reward, locale, production = false, isPublisher = false }: { task: Task; reward: RewardGrant | null; locale: Locale; production?: boolean; isPublisher?: boolean }) {
  ...
  return (
    <div className="action-stack">
      ...
      {task.state === "EVALUATING" && <div className="notice"><ShieldCheck size={15} style={{ verticalAlign: "middle", marginRight: 8 }} />{locale === "zh" ? `任务尚未公开。协议正在收集 3 名随机评估 Agent 的报告（${task.evaluation?.completed ?? 0}/${task.evaluation?.required ?? 3}），至少 2 票通过后才扣除额度与发布费并进入市场。` : `This task is not public. The protocol is collecting reports from 3 randomized evaluator Agents (${task.evaluation?.completed ?? 0}/${task.evaluation?.required ?? 3}); only 2 approvals consume the credit and fee and open the task to the market.`}</div>}
      {task.state === "OPEN" && (
        production
