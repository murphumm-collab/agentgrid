...
export const onChainActionContracts = [
  ...
  action("claim-executor-slot", "EXECUTION", "ELIGIBLE_EXECUTOR", "taskRegistry", "claimTask(uint256)", "task is open/claimed/correction, a slot is free and caller has no conflict", "adds the caller to the frozen execution team or competition set"),
  ...
  action("evict-inactive-executor", "EXECUTION", "COORDINATOR", "taskRegistry", "evictInactiveExecutor(uint256,address)", "the assigned executor missed the on-chain inactivity window without contributing", "removes the inactive member and reopens formation for the same work round"),
  ...
  action("request-validator-draw", "VERIFICATION", "COORDINATOR", "taskRegistry", "requestTester(uint256)", "task is submitted and no valid draw is already pending", "commits the eligible validator candidate set and a future selection block"),
  action("finalize-validator-draw", "VERIFICATION", "COORDINATOR", "taskRegistry", "finalizeTester(uint256)", "the future block is available within 256 blocks and three conflict-free validators remain eligible", "starts a fresh three-member isolated-shard panel epoch"),
  ...
  action("request-maintenance-panel", "MAINTENANCE", "COORDINATOR", "taskRegistry", "requestMaintenancePanel(uint256,uint8)", "checkpoint is due, ordered and not already proven", "starts a fresh current-registry validator draw; it never reuses the acceptance panel"),
