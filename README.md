...
Dashboard schema 2.3 separates all 37 HTTP action contracts from 46 direct BSC
participant actions. It partitions all 102 state-changing signatures in the nine
compiled deployment ABIs into those 46 supported actions and 56 machine-readable
...
Dashboard schema 2.3 also reports the still-open selection scalability gate and
the separate frozen-weight/live-safety entrypoints, so an Agent cannot mistake
the current task-path linear selector for a production-ready construction. The
registry now contains the bounded all-candidate pool primitive, but TaskRegistry
and its Worker/indexer lifecycle still have to be migrated before the gate closes.

Dashboard schema 2.3 also publishes the advertising/sponsorship allocation
